...
from mmdet3d.evaluation import cust_eval
...
class CustomMetric(BaseMetric):
    ...
    Args:
        ...
        default_cam_key (str): The default camera for lidar to camera
            conversion. By default, custom: 'CAM2', Waymo: 'CAM_FRONT'.
            Defaults to 'CAM2'.
    ...
    def __init__(self,
        ...
        self.default_prefix = 'custom metric'
    ...
    def convert_annos_to_custom_annos(self, data_infos: dict) -> List[dict]:
        """Convert loading annotations to custom annotations.
        ...
        Returns:
            List[dict]: List of custom annotations.
        """
            ...
            for i, annos in enumerate(data_annos):
                if len(annos['instances']) == 0:
                    ...
                    for instance in annos['instances']:
                        ...
                        custom_annos['name'].append(label2cat[label])
                        # custom_annos['truncated'].append(instance['truncated'])
                        # custom_annos['occluded'].append(instance['occluded'])
                        # custom_annos['alpha'].append(instance['alpha'])
                        # custom_annos['bbox'].append(instance['bbox'])
                        custom_annos['location'].append(instance['bbox_3d'][:3])
                        custom_annos['dimensions'].append(
                            instance['bbox_3d'][3:6])
                        custom_annos['rotation_y'].append(
                            instance['bbox_3d'][6])
                        # custom_annos['score'].append(instance['score'])
                    for name in custom_annos:
                        custom_annos[name] = np.array(custom_annos[name])
                data_annos[i]['custom_annos'] = custom_annos
        return data_annos
    ...
    def compute_metrics(self, results: List[dict]) -> Dict[str, float]:
        ...
        self.data_infos = self.convert_annos_to_custom_annos(pkl_infos)
        ...
        gt_annos = [
            self.data_infos[result['sample_idx']]['custom_annos']
    ...
    def custom_evaluate(self,
        ...
        for name in results_dict:
            ...
            # print(f'results_dict[{name}]',results_dict[name])
            if len(results_dict[name][0]['name']) == 0:
                print('pass')
                continue
            ap_result_str, ap_dict_ = cust_eval(
    ...
    def format_results(
        ...
        for name in results[0]:
            ...
            if 'pred_instances' in name and '3d' in name and name[
                ...
                result_list_ = self.bbox2result_custom(net_outputs,
                ...
                result_dict[name] = result_list_
                    ...
                    name]:
                net_outputs = [result[name] for result in results]
                result_list_ = self.bbox2result_custom2d(
                    net_outputs, sample_idx_list, classes, pklfile_prefix_,
                    submission_prefix_)
                result_dict[name] = result_list_
        return result_dict, tmp_dir

    def bbox2result_custom(
            self,
        ...
        """Convert 3D detection results to custom format for evaluation and test
        submission.
        ...
        Returns:
            List[dict]: A list of dictionaries with the custom format.
        """
        ...
        det_annos = []
        print('\nConverting 3D prediction to custom format')
    ...
    def bbox2result_custom2d(
            ...
            pklfile_prefix: Optional[str] = None,
            submission_prefix: Optional[str] = None) -> List[dict]:
        """Convert 2D detection results to custom format for evaluation and test
        submission.
        ...
        assert len(net_outputs) == len(self.data_infos), \
            'invalid list length of network outputs'
        det_annos = []
        print('\nConverting 2D prediction to custom format')
        ...
        if submission_prefix is not None:
            ...
            mmengine.mkdir_or_exist(submission_prefix)
            print(f'Saving custom submission to {submission_prefix}')
    ...
    def convert_valid_bboxes(self, box_dict: dict, info: dict) -> dict:
        ...
        print('box before filter:', box_preds)
        ...
        if isinstance(box_preds, LiDARInstance3DBoxes):
            ...
            valid_inds = valid_pcd_inds.all(-1) #  valid_cam_inds & valid_pcd_inds.all(-1)
        # else:
